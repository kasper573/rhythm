...
/// step. Selected on the mode select scene (kept on the `Game` root) and
/// read by every scene after it.
#[derive(Debug, Clone, Copy, Default, PartialEq, EnumIter, IntoStaticStr)]
pub enum PlayMode {
    #[default]
    Singles,
    Doubles,
