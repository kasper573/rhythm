...
/// The scanned library, set once by the boot sequence via
/// [`StepfileLibrary::install`].
pub fn library() -> &'static StepfileLibrary {
    LIBRARY.get().expect("StepfileLibrary installed at boot")
}

static LIBRARY: OnceLock<StepfileLibrary> = OnceLock::new();
...
impl StepfileLibrary {
    /// Scans and installs the library for [`library`] access.
    pub fn install() {
        LIBRARY
            .set(StepfileLibrary::scan())
            .unwrap_or_else(|_| panic!("StepfileLibrary is already installed"));
    }
    ...
    pub fn scan() -> StepfileLibrary {
        ...
        if group_dirs.is_empty() {
            godot_warn!("no stepfile library at {}", root.display());
...
impl StepfileEntry {
    /// This stepfile as background music for the
    /// [`MusicPlayer`](crate::core::stepfile::MusicPlayer).
    pub fn bgm(&self) -> Bgm {
        Bgm {
...
fn load_stepfile_folder(dir: &Path) -> Vec<StepfileEntry> {
    platform()
        ...
        .filter_map(|sm_path| match Stepfile::load(&sm_path) {
            ...
            Err(error) => {
                godot_warn!("skipping {}: {error}", sm_path.display());
                None
            }
        })
        .collect()
