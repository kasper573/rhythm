...
# dev box (Xvfb + lavapipe + PulseAudio); override the DRIVE_* vars for another.
#
...
#  * User data is sandboxed under $WORK/home, so ~/.config/rhythm is untouched
#    and every run starts from default settings.
#
# Commands (see the Testing section of CLAUDE.md for the loop):
...
ICD="${DRIVE_ICD:-/usr/share/vulkan/icd.d/lvp_icd.json}"
SINK="${DRIVE_SINK:-rhythm_test}"
SETTLE="${DRIVE_SETTLE:-6}"
GAME="$ROOT/target/debug/rhythm"
mkdir -p "$WORK/home"
...
start() {
  CARGO_INCREMENTAL=0 cargo build --manifest-path "$ROOT/Cargo.toml" --bin rhythm -q || return 1
  pkill -9 -x rhythm 2>/dev/null; sleep 0.5
  ...
  PULSE_SINK="$SINK" XDG_CONFIG_HOME="$WORK/home" \
  BEVY_ASSET_ROOT="$ROOT" VK_ICD_FILENAMES="$ICD" WGPU_BACKENDS=vulkan \
    "$GAME" >"$WORK/game.log" 2>&1 &
  local w=""
...
stop() { pkill -9 -x rhythm 2>/dev/null; echo stopped; }
